...
2. タイマー / ストップウォッチ
3. ルーレット (項目をランダム抽選)
4. **HTML / CSS / JavaScript の整形ツール**（それぞれ別ツール、または 1 ツールでタブ切替でも可。外部依存なしで実装する）
5. **画像加工系**（画像圧縮 / リサイズ / 白黒・グレースケール変換 / フォーマット変換 など。Canvas API で実装可。1 ツールに複数機能をタブで束ねてもよい）
6. その他: カラーピッカー / QR コード生成 / UUID 生成 / Markdown プレビュー / Unix time 変換 / 正規表現テスター / パスワード生成 / JSON 整形 / Base64 エンコード / URL エンコード / ハッシュ計算 など
