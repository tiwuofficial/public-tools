...
ツール候補は `issues/` 配下で **1 ツール = 1 Markdown ファイル** として管理する。

- 新規実装は `issues/` の未実装ツールから選ぶ。
- ここから先は **「非エンジニアでも使う一般的なツール」** を優先する（ユーザ指示）。
- 開発者専用の便利ツール（API テスト、JWT、cron 式など）は低優先。
- 実装済み・未実装・実装しないものは各 issue ファイルの「状態」で管理する。
- 新しい候補を追加する場合は `issues/<kebab-case-name>.md` を作成する。
